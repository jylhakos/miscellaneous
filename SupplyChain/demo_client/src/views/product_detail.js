...
const ROLE_TO_ENUM = {
  ...
  'delivery': payloads.createProposal.enum.DELIVERY,
...
const ProductDetail = {
  ...
  view (vnode) {
    ...
    let delivery = vnode.state.delivery
    ...
    return [
      m('.product-detail',
        ...
        _row(
          _labelProperty('Delivery', _agentLink(delivery)),
          m(TransferControl, {
            publicKey,
            record,
            agents: vnode.state.agents,
        ...
        _row(_labelProperty('Category', getPropertyValue(record, 'product_category'))),

        _row(
          /*_labelProperty('Length (m)', parsing.toFloat(getPropertyValue(record, 'length', 0))), */
          _labelProperty('Color ', getPropertyValue(record, 'product_color')),
          _labelProperty('Weight (kg)', parsing.toFloat(getPropertyValue(record, 'product_weight', 0)))),

        _row(
          _labelProperty(
            'Location',
            _propLink(record, 'product_location', _formatLocation(getPropertyValue(record, 'product_location')))
          ),
          (isReporter(record, 'product_location', publicKey) && !record.final
...
const _formatLocation = (location) => {
  if (location && location.latitude !== undefined && location.longitude !== undefined) {
    ...
    console.log('_formatLocation latitude: ' + latitude)
    console.log('_formatLocation longitude: ' + longitude)
