...
const ProductList = {
  oninit (vnode) {
    ...
    const refresh = () => {
      api.get('records?recordType=product').then((records) => {
        ...
        m(Table, {
          ...
          rows: vnode.state.filteredRecords.slice(
            ...
            (vnode.state.currentPage + 1) * PAGE_SIZE)
                .map((rec) => [
                  m(`a[href=/product/${rec.recordId}]`, {
                    oncreate: m.route.link
                  }, truncate(rec.recordId, { length: 32 })),
                  getPropertyValue(rec, 'product_category'),
      ...
      m('.col-sm-8',
        m(FilterGroup, {
          ...
          filters: {
            ...
            'Delivery': () => filterRecords((record) => record.delivery === publicKey),
