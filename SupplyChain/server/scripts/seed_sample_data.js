...
protos.compile()
  ...
  // Transfer Delivery
  .then(() => {
    console.log('Transferring Delivery . . .')
    const deliveryProposals = records
      .filter(record => record.deliveryIndex !== undefined)
      .map(record => {
        return createProposal(agents[record.ownerIndex || 0].privateKey, {
          recordId: record.recordId,
          receivingAgent: agents[record.deliveryIndex].publicKey,
          role: protos.Proposal.Role.DELIVERY
        })
      })

    return submitTxns(deliveryProposals)
  })
  .then(() => {
    const deliveryAnswers = records
      .filter(record => record.deliveryIndex !== undefined)
      .map(record => {
        return answerProposal(agents[record.deliveryIndex].privateKey, {
          recordId: record.recordId,
          receivingAgent: agents[record.deliveryIndex].publicKey,
          role: protos.Proposal.Role.DELIVERY,
    ...
    return submitTxns(deliveryAnswers)
  })
