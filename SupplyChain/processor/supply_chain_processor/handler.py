...
def _finalize_record(payload, signer, timestamp, state):
    '''
    * Check that the signer is both the owner and delivery
    * Check that the record is not already final
    ...
    if not _is_owner(record, signer) or not _is_delivery(record, signer):
        raise InvalidTransaction(
            'Must be owner and delivery to finalize record')
...
def _create_proposal(payload, signer, timestamp, state):
    ...
    if role == Proposal.DELIVERY:
        if not _is_delivery(record, signer):
            raise InvalidTransaction(
                'Must be delivery')
...
def _accept_proposal(state, signer, proposal, timestamp):
    ...
    if role == Proposal.OWNER:
    ...
    elif role == Proposal.DELIVERY:
        if not _is_delivery(record, issuing_agent):
            LOGGER.info('Issuing agent is not delivery')
            return Proposal.CANCELED
        ...
        record.deliveries.sort(key=lambda agent: agent.timestamp)
...
def _is_delivery(record, agent_id):
    return record.deliveries[-1].agent_id == agent_id
