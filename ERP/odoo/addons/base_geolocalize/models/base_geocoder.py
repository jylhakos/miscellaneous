...
class GeoCoder(models.AbstractModel):
    ...
    def geo_find(self, addr, **kw):
        """Use a location provider API to convert an address string into latitude, longitude tuple.
    ...
    @api.model
    def _call_base_openstreet(self, addr, **kw):
        """
        ...
        try:
            ...
            _logger.info('base_openstreet nominatim service called')
            if response.status_code != 200:
                _logger.error('Request to base_openstreet failed.\nCode: %s\nContent: %s' % (response.status_code, response.content))
